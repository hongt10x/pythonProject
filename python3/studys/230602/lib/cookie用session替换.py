...
def login(in_data):
    ...
    resp = session.post(url, data=in_data)
...
class Lesson:
    def lesson_list(self, in_data):
        ...
        resp = session.get(url, params=in_data)
        resp.encoding = 'unicode_escape'
...
'''
响应内容--->  {"retcode": 0}
响应头数据--->  {'Content-Type': 'application/json', 'X-Frame-Options': 'SAMEORIGIN', 'Content-Length': '14', 'Vary': 'Cookie', 'Set-Cookie': 'sessionid=2qa1wxv0artwsareya9875jg5118uxyl; HttpOnly; Path=/', 'Date': 'Mon, 13 May 2024 07:12:00 GMT', 'Server': '0.0.0.0'}
响应cookies--->  <RequestsCookieJar[<Cookie sessionid=2qa1wxv0artwsareya9875jg5118uxyl for 124.223.33.41/>]>
