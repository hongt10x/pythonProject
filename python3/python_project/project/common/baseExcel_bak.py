...
class BaseExcel(object):
    ...
    def compare_status(self, update_sh, max_row):
        ...
        compare_type = inspect.stack()[0][3]
        sheet_info = inspect.stack()[1][3].split("_")[-1]
        print(f"sheet_info: {self.config[sheet_info]['sheetName']}")
        ...
        print(f"config: {self.config[sheet_info][compare_type]}")
        ...
        for i in range(2, max_row + 1):
            ...
            for column in range(compare_column):
                ...
                if compare_logic and compare_logic == 'AND':
                    ...
                    if cell_value in sh_texts and cell_value not in invalid_infos:
    ...
    def compare_status_0607(self, update_sh):
        ...
        compare_type = inspect.stack()[0][3]
        sheet_info = inspect.stack()[1][3].split("_")[-1]
        sh = self.wb[self.config[sheet_info]['sheetName']]
        ...
        print(f"  查询{sh.title}表中列项数据:{pattern}<-->列项索引:{pattern_indexs}")
        print(f"  更新{update_sh.title}表中列项数据:{update_info}<-->列项索引:{update_indexs}")
        self.clean_col_values(update_indexs[1:], update_info[1:], update_sh)

        # sys.exit(1)
        # 获取OS表第一列比对数据
        update_texts = self.get_col_values(update_indexs[0], update_info[0], update_sh)
        sh_texts = self.get_col_values(pattern_indexs[0], pattern[0], sh)
        # print(f"os_texts: {update_texts}")
        # print(f"sh_texts: {sh_texts}")

        for i, value in enumerate(update_texts):
            v_index = None
            if sheet_info == "antivirus":  # 将防病毒库中的数据状态更新到OS表中;# 防病毒一个单元格有多个ip，需要处理
                ...
                new_sh_texts = self.recombine_texts(sh_texts, pattern, sh)
                # print(f"new_sh_texts: {new_sh_texts}")
                if value in new_sh_texts.keys():
                    # print(f"value{i}: {value}")
                    v_index = new_sh_texts.get(value)  # 将行号取出
                    # for idx in range(len(update_indexs) - 1):
                    #     # self.get_col_values返回值为列表还剔除了首行，所以需要下移2位，保持索引有效 # 拿到下一列数据对应的值
                    #     next_value = self.get_col_values(pattern_indexs[idx + 1], pattern[idx + 1], sh)[v_index]
                    #     print(f"next_value: {next_value}")
                    #     update_sh.cell(row=i + 2, column=update_indexs[idx + 1], value=next_value)
            elif value in sh_texts:
                v_index = sh_texts.index(value)  # 拿到值的索引,索引从0开始，表格更新从1开始
            # print(f"v_index: {v_index}")
            if v_index is not None:
                for idx in range(len(update_indexs) - 1):
                    # self.get_col_values返回值为列表还剔除了首行，所以需要下移2位，保持索引有效
                    # next_value = self.get_col_values(pattern_indexs[idx+1], pattern[idx+1], sh)[v_index-2]  # 拿到下一列数据对应的值
                    next_value = self.get_col_values(pattern_indexs[idx + 1], pattern[idx + 1], sh)[
                        v_index]  # 拿到下一列数据对应的值
                    # print(f"next_value11: {next_value}")
                    update_sh.cell(row=i + 2, column=update_indexs[idx + 1], value=next_value)
        ...
        for row in range(2, sh_max_row + 1):
            ...
            for column in range(compare_column):
                ...
                if sheet_info == 'titan' and column == 0:  # 对青藤云的多列数据重组，获取业务IP和带内IP
                    # print(f"help_pattern: {help_pattern}")
            ...
            if match_times == compare_column:
                ...
                for i, update_index in enumerate(update_indexs):
                    ...
                    if sheet_info == 'automation' and i == refer_index and update_value:  # 对自动化表的主机名数据拆分
                        try:
                            dn_ip = f'{update_value.split("_")[1]}'  # 先拿到带内IP
                        except:
                            dn_ip = None
    ...
    def compare_datas_0605(self, update_sh, max_row):
        # 获取被调用函数名
        # print(inspect.stack()[1][3])
        # 获取当前函数名
        compare_type = inspect.stack()[0][3]
        ...
        # 获取比对列数据--目前设定为第列元素
        sh_texts = self.get_col_values(pattern_indexs[0], pattern[0], sh)
        # 获取OS表第一列比对数据，pattern[0]数据无影响
        update_texts = self.get_col_values(update_indexs[0], update_info[0], update_sh)
        # 将防病毒库中独有的数据更新到OS表中
        record_line = 1
        for num, value in enumerate(sh_texts):
            if value not in update_texts:
                # 如果未查到数据则获取第二列数据进行比对
                for i, update_index in enumerate(update_indexs):
                    if update_index == update_indexs[-1]:
                        # 最后一列更新数据注明数据来源，行往后追加
                    ...
                    # print(f"update_value1: {update_value}")
                    if sheet_info == 'host' and i == refer_index and update_value:  # 对主机表的操作系统数据拼接
    ...
    def clean_col_values(self, indexs, pattern, sh):
        ...
        if indexs:
            if type(indexs) is list:
                ...
                # for index in indexs:
                #     sh.delete_cols(index)
                #     sh.delete_cols(index)  # 清除已有列数据，存在清理不干净情况，再次执行
            else:
